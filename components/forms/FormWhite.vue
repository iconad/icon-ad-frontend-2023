  ...
  <div class="form-white">
      <form @submit.prevent="submitForm" class="form">
        ...
        <div class="select-none transition-all duration-100 focus:outline-none active:outline-none active:border-none group flex flex-wrap items-center h-full">
          <span :class="captcha.status ? 'bg-green-200' : 'bg-theme-gray-dark'" class="bg-opacity-75 px-4 py-2 transition-all rounded-xl block mr-3 flex items-center justify-between space-x-2 w-[50%] md:w-[30%] mb-3 md:mb-0">
          ...
          <div v-if="captcha" class="w-full md:w-4/6 select-none font-medium right-0 text-sm text-left" :class=" captcha.success ? 'text-green-500' : captcha.error ? 'text-theme-red' : 'text-gray-100'">
            {{captcha.message}}
          </div>
        </div>
        <!-- Captcha End -->
        ...
        <div v-if="!isLoading" class="form-element">
          <span v-if="captcha && !captcha.status" class="text-center select-none form-button block black-gray w-full cursor-not-allowed hover:bg-opacity-100"> Submit </span>
