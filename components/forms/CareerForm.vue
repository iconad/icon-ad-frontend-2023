  ...
  <div>
       <form @submit.prevent="submitResume" class="form">
        ...
        <div class="form-element">
          <client-only>
            <vue-tel-input
              class="form-element text-gray-700"
              required
       ...
       <div class="form-element">
        ...
        <div class="select-none transition-all duration-100 focus:outline-none active:outline-none active:border-none group flex flex-wrap items-center h-full">
          <span :class="captcha.status ? 'bg-green-200' : 'bg-theme-gray-dark'" class="bg-opacity-75 px-4 py-2 transition-all rounded-xl block mr-3 flex items-center justify-between space-x-2 w-1/3">
            <span class="block">
              {{captcha.num1}}
              {{captcha.opreator}}
              {{captcha.num2}}
            ...
            <span>
              <input type="text" v-model="input" :disabled="captcha.status" @input="checkCaptcha" class="w-10 rounded-full text-center focus:outline-none" :class="captcha.status ? 'select-none bg-green-200' : 'bg-theme-gray-1'" placeholder="?">
            </span>
          </span>
          <div v-if="captcha" class="w-3/5 select-none font-medium right-0 text-sm text-left" :class=" captcha.success ? 'text-green-500' : captcha.error ? 'text-theme-red' : 'text-gray-100'">
            {{captcha.message}}
          </div>
        </div>
        <!-- Captcha End -->
        ...
        <div v-if="!isLoading" class="form-element flex justify-center w-full">
          <span v-if="captcha && !captcha.status" class="select-none form-button block rainbow-gray-dark w-full cursor-not-allowed hover:bg-opacity-100 text-center"> Submit </span>
          <input v-else type="submit" class="mx-auto form-button rainbow cursor-pointer hover:bg-opacity-100" value="Submit">
        </div>
        <div v-else>
